...
// ===== Helper to fetch Hotel Names linked to the package =====
function getHotelNames($conn, $h_ids) {
    if (empty(trim($h_ids))) return '<span class="text-muted">No hotels mapped</span>';
    $cleaned = preg_replace('/[^0-9,]/', '', $h_ids);
    if (empty($cleaned)) return '<span class="text-muted">No hotels mapped</span>';
...
if ($status_filter !== 'all') {
    ...
    $where .= " AND pb.status = '$st'";
}

if ($search) {
    $s = mysqli_real_escape_string($conn, $search);
    $where .= " AND (pb.booking_ref LIKE '%$s%' OR pb.payment_id LIKE '%$s%' OR cd.cust_fname LIKE '%$s%' OR cd.cust_lname LIKE '%$s%' OR p.pa_name LIKE '%$s%')";
}

// SQL Query with LEFT JOINS to fetch Customer & Package details
$query = "SELECT pb.*, cd.cust_fname, cd.cust_lname, cd.cust_email, cd.cust_mobile, p.pa_name, p.h_id as package_hotels
          FROM predefine_booking pb
          LEFT JOIN customer_details cd ON pb.cust_id = cd.cust_id
          LEFT JOIN package p ON pb.pa_id = p.pa_id
          $where
...
// Stats calculations
$stats_total = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM predefine_booking"));
$stats_pending = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM predefine_booking WHERE status='Pending'"));

$rev_res = mysqli_query($conn, "SELECT SUM(amount) as total FROM predefine_booking WHERE status='Confirmed'");
$total_revenue = mysqli_fetch_assoc($rev_res)['total'] ?? 0;

$page_title = "Special Bookings"; // Sets Topbar Title dynamically
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Special Bookings – Explore India Admin</title>
...
<div class="main-content">
    <div class="content-wrap">
        ...
        <div class="page-header">
            <h1>Special Bookings Directory</h1>
            <p>Monitor package tour bookings, confirm payment gateways, and view mapped package hotels.</p>
        </div>
        ...
        <div class="za-card">
            <div class="za-card-header">
                <div class="za-card-title">Predefined Package Bookings Log (<?= $total_rows ?>)</div>
                
                <div style="display:flex; gap:12px; align-items:center; flex-wrap:wrap;">
                    <!-- Filter bar -->
                    <div class="za-filter-bar">
                        <a href="predefine_bookings_manage.php?status=all&search=<?= urlencode($search) ?>" class="za-filter-pill <?= $status_filter=='all'?'active':'' ?>">All</a>
                        <a href="predefine_bookings_manage.php?status=Confirmed&search=<?= urlencode($search) ?>" class="za-filter-pill <?= $status_filter=='Confirmed'?'active':'' ?>">Confirmed</a>
                        <a href="predefine_bookings_manage.php?status=Pending&search=<?= urlencode($search) ?>" class="za-filter-pill <?= $status_filter=='Pending'?'active':'' ?>">Pending</a>
                    </div>
            ...
            <div style="padding:0; overflow-x:auto;">
                    ...
                    <table class="za-table">
                        ...
                        <tbody>
                            <?php while ($c = mysqli_fetch_assoc($result)): ?>
                                ...
                                $hotel_names_str = getHotelNames($conn, $c['package_hotels']);
                                ?>
                                <tr>
                                    ...
                                    <td style="font-weight:600; color:var(--text);"><?= htmlspecialchars($c['pa_name'] ? $c['pa_name'] : 'Unknown Package') ?></td>
                                    ...
                                    <td>
                                        ...
                                        <?php else: ?>
                                            <span class="za-badge red"><i class="fa fa-times-circle"></i> <?= htmlspecialchars($c['status']) ?></span>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <!-- Open details modal passing joined database values -->
                ...
                <?php else: ?>
                    <div class="za-empty">
                        ...
                        <p>No package bookings found matching the parameters.</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
...
<div class="za-modal-overlay" id="bookingModal">
    <div class="za-modal">
        <div class="za-modal-header">
            <div class="za-modal-title">📋 Predefined Booking Details</div>
        ...
        <div class="za-modal-body">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 20px; padding-bottom: 15px; border-bottom: 1px solid var(--border);">
                <div>
                    ...
                    <h3 id="view_ref" style="font-family:'Montserrat',sans-serif; font-size: 20px; font-weight: 800; color: var(--dark); margin-top: 2px;">PRE000000</h3>
                </div>
            ...
            <div style="margin-bottom: 20px;">
                <label class="za-label">Booked Package</label>
                <div style="background: #F8FAFC; border: 1.5px solid var(--border); border-radius: 12px; padding: 14px 18px;">
                    <div style="font-weight: 700; color: var(--dark); font-size: 15px; margin-bottom: 6px;" id="view_package_name">Package Name</div>
                    <div style="font-size: 13px; color: var(--text); line-height: 1.5;">
                        Included Hotels: <strong id="view_hotels" style="color:var(--orange);"></strong>
                    </div>
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr; gap: 16px; border-top: 1px solid var(--border); padding-top: 20px;">
                ...
                <div>
                    <label class="za-label">Booking Created At</label>
...
function openBookingModal(b, hotels) {
    ...
    document.getElementById('view_package_name').innerText = b.pa_name ? b.pa_name : 'Unknown Package';
